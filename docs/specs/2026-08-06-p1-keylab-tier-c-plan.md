...
Date: 2026-08-06
Status: planned to step level. Depends on nothing. Blocks P2 and P4.
...
- [ ] Add to `crates/keylab/src/encode.rs`:
...
- [ ] Add key packing, with every shift named explicitly rather than inline:
...
- [ ] `latency_boundaries_are_exact` — 0, 149, 150, 399, 400, 999, 1000, 60000
- [ ] `ngram_packing_round_trips` — every combination of edge values, including `POS_ABSENT` and `POS_UNATTRIBUTED`
- [ ] `pins_ngram_bit_layout` — asserts the packed value of one known tuple, so a shift change cannot pass silently

## Task 2 — configuration

- [ ] Add to `crates/keylab/src/config.rs`:
...
- [ ] Add `Config` fields: `ngram_capture: bool`, `ngram_seal_count: u32`, `ngram_min_count: u32`
- [ ] `Default`: `true`, `MIN_NGRAM_SEAL_COUNT`, `MIN_NGRAM_MIN_COUNT`
- [ ] `validate()`: reject `ngram_seal_count < MIN_NGRAM_SEAL_COUNT` and `ngram_min_count < MIN_NGRAM_MIN_COUNT`, with the same message shape as the existing floor checks (`config.rs:121-132`)
...
**Tests**
- [ ] `rejects_lower_ngram_seal_count`
- [ ] `rejects_lower_ngram_min_count`
- [ ] `defaults_match_the_privacy_spec` — extend the existing test at `config.rs:229`
- [ ] `a_config_without_ngram_keys_still_loads` — parse a TOML string with no `ngram_*` keys and assert the defaults

## Task 3 — aggregator state and capture

All in `crates/keylab/src/aggregate.rs`.

- [ ] Add a `PendingNgram` struct: `ring: [u8; NGRAM_N]`, `mod_mask: u8`, `latency: u8`. Implement `zeroize` and `Drop`.
...
- [ ] In `key_down` (`aggregate.rs:220`), after `pos_index` is computed and **only when `code != KEY_BACKSPACE`**, shift `ngram_ring` left and append `pos_index as u8`.
- [ ] In `key_down`, inside the existing `if code == KEY_BACKSPACE { if self.bsp_run == 0 { … } }` block (`aggregate.rs:241-254`), set `pending_ngram` from the current ring, the active mod mask, and `latency_bucket(t_ms - last_keydown_ts)`.
- [ ] Add `fn active_mod_mask(&self) -> u8` deriving the mask from the keys of `keys_since_mod_down`, which are exactly the mod classes currently held.
- [ ] In `finish_backspace_run` (`aggregate.rs:344`), after the existing `BSP_BURST` / `BSP_BURST_AFTER_MOD` increments, commit the pending record when capture is enabled.
...
**Tests**
- [ ] `ngram_ring_holds_the_three_preceding_positions` — type four keys then backspace; assert the ring is keys 2,3,4
- [ ] `backspace_never_enters_the_ring` — two backspaces in a run must not shift the ring
- [ ] `run_bucket_is_resolved_at_run_end_not_start` — a three-press run records `run_bucket(3)`, not `run_bucket(1)`
- [ ] `latency_uses_the_gap_before_the_backspace` — assert bucket boundaries at 149/150 and 999/1000 ms
- [ ] `a_short_stream_records_absent_positions` — one key then backspace yields two `POS_ABSENT` slots
- [ ] `capture_disabled_records_nothing`

## Task 4 — bounds, discard, and resync

- [ ] Extend `bounded_footprint()` (`aggregate.rs:474`) to include both new maps.
- [ ] Extend `footprint_bound()` (`aggregate.rs:489`) by `(MAX_NGRAM_ENTRIES + MAX_NGRAM_FINGER_ENTRIES) * profile_count.clamp(1, MAX_PROFILES)`.
- [ ] On insert into `ngram_accum` when the map is at `MAX_NGRAM_ENTRIES` and the key is new: insert into `ngram_finger_accum` instead, projecting positions through the keymap.
- [ ] On insert into `ngram_finger_accum` when *that* map is at `MAX_NGRAM_FINGER_ENTRIES` and the key is new: increment `ngram_dropped[profile]`.
- [ ] Extend `discard_partials` (`aggregate.rs:434`) to zeroize the ring, `pending_ngram`, both maps, `ngram_events`, `ngram_dropped`, and `ngram_start_ts`.
- [ ] Extend `resync_after_sequence_loss` (`aggregate.rs:461`) to clear the ring and `pending_ngram` **but not** the accumulators.
...
**Tests**
- [ ] `ngram_footprint_stays_within_the_bound` — extend the existing 200k-event test at `aggregate.rs:922-935`
- [ ] `overflowing_the_position_map_degrades_to_fingers`
- [ ] `overflowing_the_finger_map_counts_as_dropped`
- [ ] `discard_clears_the_ring_and_pending_record` — extend `discard_zeroes_both_partial_tiers_and_context` (`aggregate.rs:731`)
- [ ] `resync_clears_the_ring_but_keeps_accumulators`

## Task 5 — seal

- [ ] Add `TierCSeal` with `Drop`-based zeroization, mirroring `TierBSeal` (`aggregate.rs:108-120`):
...
- [ ] Seal when `ngram_events[profile] >= ngram_seal_count`. At seal, partition `ngram_accum[profile]`: entries with `n >= ngram_min_count` become `rows`; the rest are projected through the keymap to finger triples and merged into `finger_rows` together with `ngram_finger_accum[profile]`.
- [ ] Change `handle_event` to return a `SealBatch { tier_b: Option<TierBSeal>, tier_c: Option<TierCSeal> }` instead of `Option<TierBSeal>`.
...
**Tests**
- [ ] `seal_preserves_the_correction_total` — the exact-sum invariant, over a randomised stream
- [ ] `seal_suppresses_rows_below_the_minimum_count`
- [ ] `suppressed_rows_appear_in_the_finger_projection`
- [ ] `seal_clears_only_the_sealing_profile`
- [ ] `no_seal_below_the_configured_count`

## Task 6 — persistence and schema v4

All in `crates/keylab/src/store.rs`.

- [ ] Add the three tables to `SCHEMA` (`store.rs:13`), following the existing `WITHOUT ROWID` style used by `pos_count` (`store.rs:96`):
...
- [ ] Add `pub fn seal_tier_c(&mut self, device_id, profile_id, seal: &TierCSeal) -> Result<()>`, modelled on `seal_tier_b` (`store.rs:278`), in one transaction.
- [ ] Add `fn migrate_v3_to_v4(connection: &Connection) -> Result<()>` — pure `CREATE TABLE IF NOT EXISTS`, no backfill, then `UPDATE meta SET value = '4' WHERE key = 'schema_version'`, then `info!("migrated database schema from version 3 to 4")`.
- [ ] Update the version match (`store.rs:459-467`) to `None | Some("4") => {}`, `Some("1")` → v1→v2→v3→v4, `Some("2")` → v2→v3→v4, `Some("3")` → v3→v4, and the seed literal at `store.rs:470` to `'4'`.
...
**Tests**
- [ ] `seals_a_tier_c_window`
- [ ] `refuses_a_tier_c_window_below_the_floor`
- [ ] `refuses_an_inconsistent_tier_c_window`
- [ ] `migrates_v3_to_v4` — build a v3 database, open, assert version 4 and that the three tables exist
- [ ] `migrates_v1_all_the_way_to_v4` — extend the existing v1 migration test (`store.rs:850`)

## Task 7 — daemon wiring

All in `crates/keylab/src/main.rs`.

- [ ] Update the `handle_event` call site (`main.rs:599-612`) for `SealBatch`.
- [ ] Add `translate_tier_c_timestamps`, mirroring `translate_tier_b_timestamps` (`main.rs:610`), so `start_ts`/`end_ts` are wall-clock.
- [ ] Persist via `store.seal_tier_c(runtime.device_id, runtime.profile_id, &seal)`.
- [ ] Thread `config.ngram_capture`, `config.ngram_seal_count` and `config.ngram_min_count` to the aggregator on the same path as `config.tier_b_seal_count`.
- [ ] Update the footprint `debug_assert!` (`main.rs:613-616`) — it should need no change if `footprint_bound` was extended correctly. Confirm rather than assume.
...
**Tests**
- [ ] Extend the existing daemon-level tests (`main.rs:897`, `:962`) to assert a Tier C window appears after enough corrections and that a hard pause mid-window discards it.

## Task 8 — documentation

- [ ] `docs/keylab.md:5-19` "What is recorded": add Tier C with its floor and its degradation rule.
- [ ] `docs/keylab.md:15`: the sentence claiming keylab records no bigrams, no words and no event order is now **false**. Replace it with an accurate statement — keylab records no characters, no raw keycodes, no raw event log and no word identity, but it does record an ordered trigram of physical positions at each correction.
- [ ] `docs/keylab.md:217-224` schema table: add the `v3 → v4` row, backfill "none — new tables".
- [ ] `docs/keylab.md:51-67` Configure: document `ngram_capture`, `ngram_seal_count`, `ngram_min_count` and their enforced minimums.
- [ ] `docs/keylab.md:256-275` "What this does not protect against": add the four residual risks from the design document, in full.
- [ ] Add a short rationale — why correction context is recorded at all, and why capture is on by default. Compact: a paragraph, not a section.
- [ ] Check `crates/keylab/` for an example configuration installed by `install.sh` and add the new keys there if one exists.
